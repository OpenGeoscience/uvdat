...
def flood_scenario_1(simulation_result_id, network, elevation_data, flood_area):
    from uvdat.core.models import Network, RasterData, SimulationResult, VectorData

    result = SimulationResult.objects.get(id=simulation_result_id)
    try:
        network = Network.objects.get(id=network)
        elevation_data = RasterData.objects.get(id=elevation_data)
        flood_area = VectorData.objects.get(id=flood_area)
    except Exception:
...
def segment_curbs(simulation_result_id, imagery_layer):
    ...
    from uvdat.core.models import Dataset, FileItem, RasterData, SimulationResult

    result = SimulationResult.objects.get(id=simulation_result_id)
    try:
        imagery_layer = RasterData.objects.get(id=imagery_layer)
    except Exception:
        result.error_message = 'Object not found.'
        result.save()
        return

...
TEMP_TABLE_QUERY = """
CREATE TEMP TABLE excluded_nodes(id INT);
CREATE UNIQUE INDEX excluded_nodes_id_idx ON excluded_nodes (id);
INSERT INTO excluded_nodes(id) VALUES (unnest(%(excluded_nodes)s::int[])) ON CONFLICT DO NOTHING;

CREATE TEMP TABLE excluded_edges(id INT);
CREATE UNIQUE INDEX excluded_edges_id_idx ON excluded_edges (id);

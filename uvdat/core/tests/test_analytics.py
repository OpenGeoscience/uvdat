...
def test_flood_analysis_chain(project):
    ...
    assert flood_dataset.name == 'Parabolic 1 in 100 year (1% chance) Flood Simulation for Test'
    ...
    assert len(failures['0']) == 51
    assert len(failures['1']) == 82
    ...
    assert len(recoveries) == 83
    assert len(recoveries['0']) == 82
    assert len(recoveries['82']) == 0
